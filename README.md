...
A fully cloud-native data analytics dashboard built on **Amazon Web Services (AWS)**, visualizing global Electric Vehicle (EV) adoption behavior across 50,000 records.
...
> The dashboard fetches live data directly from AWS Athena via a REST API — no manual data upload required.
...
- Process and query it using serverless tools
- Display insights on a live, publicly accessible web dashboard
...
| **Amazon Athena** | Serverless querying | Runs SQL queries directly on S3 data — no database setup needed |
...
| AWS SDK | S3, Athena, Lambda integration |
...
3. **AWS Athena** queries the data using SQL (`SELECT` on the Glue table)
...
| File | Description |
|---|---|
| `ev_dashboard_live.html` | Main dashboard — fetches live data from AWS API |
| `ev_dashboard.html` | Offline version — upload CSV manually |
...
7. Install Apache: `sudo yum install -y httpd && sudo service httpd start`
...
- Lambda is within Free Tier (1M requests/month free)
- API Gateway is within Free Tier (1M calls/month free)
